...
describe("ballotPaperTest", () => {
  ...
  test("setInvalid", async () => {
    ...
    let oldCandidate1 = JSON.parse(
      JSON.stringify(candidateStore.getByID(candidate1.c_id)),
    ) as Candidate;
    let oldCandidate2 = JSON.parse(
      JSON.stringify(candidateStore.getByID(candidate2.c_id)),
    ) as Candidate;

    const wrapper = mount(_ballotPaperCandidateComponent, {
      props: {
        data: ballotStore.ballotPapers[0],
        hideIrrelevantThings: false,
        changeAllowed: true,
      },
    });
    ...
    expect(candidateStore.getByID(oldCandidate1.c_id)?.firstVotes).toBe(
      oldCandidate1.firstVotes - 1,
    );
    expect(candidateStore.getByID(oldCandidate1.c_id)?.points).toBe(
      oldCandidate1.points - 2,
    );

    expect(candidateStore.getByID(oldCandidate2.c_id)?.firstVotes).toBe(
      oldCandidate2.firstVotes,
    );
    expect(candidateStore.getByID(oldCandidate2.c_id)?.points).toBe(
      oldCandidate2.points - 1,
    );
  });
  ...
  test("noChangesWhenInvalid", async () => {
    ...
    expect(candidateStore.getByID(oldCandidate1.c_id)?.firstVotes).toBe(
      oldCandidate1.firstVotes,
    );
    expect(candidateStore.getByID(oldCandidate1.c_id)?.points).toBe(
      oldCandidate1.points,
    );
