...
function handleFileChangeBallots(event: any) {
  ballotFile.value = event.target.files[0];
}
  ...
  if (candidateFile && ballotFile) {
    ...
    readerCandidate.onload = () => {
      if (typeof readerCandidate.result === "string") {
        const candidateArray = JSON.parse(
          readerCandidate.result,
        ) as Candidate[];
        const invalidCand = candidateArray.splice(
          candidateArray.findIndex((value) => value.c_id == 0),
          1,
        );
    ...
    readerBallotPaper.readAsText(ballotFile.value!);
  } else {
...
let candidateFile: Ref<File | null> = ref(null);
let ballotFile: Ref<File | null> = ref(null);

let importReady = computed(() => {
  return candidateFile.value !== null && ballotFile.value !== null;
});
</script>
...
<template>
  <div id="settings">
    ...
    ><input type="file" @change="handleFileChangeCandidate($event)" />
    <span>Ballots: </span
    ><input type="file" @change="handleFileChangeBallots($event)" />
    <button @click="importData" :disabled="!importReady" id="import">
